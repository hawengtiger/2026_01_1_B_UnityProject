...
{
    public GameObject[] fruitPrefabs;           //과일 프리패 배열 선언
    ...
    public Camera mainCamera;                   //카메라 참조 (마우스 위치 반환에 필요)
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButton(0) && fruitTimer == -3.0f)  //마우스 좌 클릭하면 과일을 떨어트린다. && 타이머 시간은 -3.0f
        {
            DropFruit();
        }
    }
    ...
    void SpawnNewFruit()
    {
        if (!isGameOver)        //게임 오버가 아닐 때만 새 광리 생성
        {
            ...
            Vector3 spawnPosition = new Vector3(worldPosition.x, fruitStartheight, 0);      //x 좌표만 사용하고 나머지는 설정한 값으로 한다.

            float halfFruitSize = fruitSizes[currentFruitType] / 2f;

            spawnPosition.x = Mathf.Clamp(spawnPosition.x, -gameWidth / 2 + halfFruitSize, gameWidth / 2 - halfFruitSize);      //x의 위치가 게임 영역을 벗어나지 않도록 제한 
            ...
            if (rb != null)
            {
                rb.gravityScale = 0f;       //시작 시에는 중력 스케일을 0으로 해준다.
            }
    ...
    {
        if (fruitType < fruitPrefabs.Length - 1)        //마지막 과일이 아니라면
        {
            GameObject newFruit = Instantiate(fruitPrefabs[fruitType + 1], position, Quaternion.identity);      //해당 타입 다음 번호를 생성 시킨다.
