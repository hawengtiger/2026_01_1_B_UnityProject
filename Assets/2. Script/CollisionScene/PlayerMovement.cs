...
public class PlayerMovement : MonoBehaviour
{
    ...
    public float lowMultiplier = 2.0f;              //짧은 점프 배율
    ...
    void Update()
    {
        ...
        else
        {
            ...
            {
                rb.linearVelocity += Vector3.up * Physics.gravity.y * (lowMultiplier - 1) * Time.deltaTime;             //상승 중 점프 버튼을 때면 낮게 점프
            }
        }
    ...
    private void OnCollisionEnter(Collision collision)      //지면과의 충돌이 유지 되는지 확인
    {
        if (collision.gameObject.tag == "Ground")            //만약 충돌이 일어난 물체의 Tag가 Ground일 경우
        {
            realGrouned = true;      //땅과 충돌하면 True로 변경함,
    ...
    private void OnCollisionExit(Collision collision)       //지면에서 떨어졌는지 확인
    {
        if (collision.gameObject.tag == "Ground")            //만약 충돌이 일어난 물체의 Tag가 Ground일 경우
        {
            realGrouned = true;      //지면에서 떨어졌기 때문에 flase
